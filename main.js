    ...
    function navigate(path) {
      // Try pushState first (works with proper server)
      if (window.location.protocol !== 'file:') {
        window.history.pushState({}, '', path);
        window.dispatchEvent(new Event('popstate'));
      } else {
        // Fallback to hash navigation for file:// protocol
    ...
    function FloatingNav() {
      ...
      useEffect(() => {
        const updateCurrentPage = () => {
          ...
          if (path === '/' && hash === '#about') {
            ...
            setCurrentPage('about'); // Default to about on home page
          ...
          console.log('Current page detected:', path, hash, '-> setting to:', 
            path.includes('/websites') ? 'websites' : 
            path.includes('/ai') ? 'ai' : 
            path.includes('/social') ? 'social' : 'about');
        };
      ...
      const handleClick = (e, item) => {
        ...
        // Update current page immediately on click
      ...
      return (
        <>
            ...
            <div className="glass-premium rounded-full px-8 py-4 flex items-center gap-8 shadow-2xl">
              ...
              <div className="hidden md:flex gap-6 text-sm font-medium">
                {items.map((item) => {
                  ...
                  const isActive = itemLower === currentPage;
                  
                  console.log(`Nav item: ${item}, itemLower: ${itemLower}, currentPage: ${currentPage}, isActive: ${isActive}`);
          ...
          <button
            ...
            className={`lang-switch glass-premium rounded-full px-4 py-3 shadow-2xl hover:scale-105 active:scale-95 transition-all duration-300 focus:outline-none fixed top-6 right-6 z-50 ${scrolled ? 'scale-95' : 'scale-100'}`}
            style={{ 
              fontSize: '0.875rem',
              fontWeight: '500',
              cursor: 'pointer'
    ...
    function About() {
      ...
      const data = {
        heading1: aboutData?.dataset.heading1 || '',
        heading2: aboutData?.dataset.heading2 || '',
        text1: aboutData?.dataset.text1 || '',
        text2: aboutData?.dataset.text2 || ''
      };
      const features = aboutData ? JSON.parse(aboutData.dataset.features || '[]') : [];
        ...
        <section id="about" className="py-24 px-6" style={{ background: 'var(--surface)' }}>
          <div className="max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-16 items-start">
              ...
              <div className="grid gap-6 stagger-item" style={{ animationDelay: '0.2s' }}>
                {features.map((feat, idx) => (
                  ...
                  >
                    <div className="feature-icon-wrapper" style={{ color: feat.color }}>
                      {icons[feat.icon]}
                    </div>
                    ...
                    <div 
                      ...
                      style={{ 
                        background: `radial-gradient(circle at center, ${feat.color}15 0%, transparent 70%)` 
                      }}
                    ></div>
                  </div>
                ))}
    ...
    function SpotlightStack() {
      ...
      const heading = mockupData?.dataset.heading || '';
      const body = mockupData?.dataset.body || '';
      const image = mockupData?.dataset.image || '';
    ...
    function WhyUsPricing() {
      ...
      const [hasAnimated, setHasAnimated] = React.useState([false, false, false]);
              ...
              <div className="space-y-24">
                ...
                <div className="grid md:grid-cols-2 gap-12 items-center">
                  ...
                  <div className="stagger-item md:order-2 order-1" style={{ animationDelay: '0.15s' }}>
                    ...
                    <ul className="space-y-3">
                      <li className="flex items-start gap-3">
                        <span style={{ color: 'var(--accent-blue)', fontSize: '1.25rem' }}>✓</span>
      ...
      return (
        <div className="min-h-screen">
          ...
          <section ref={heroRef} className="service-page-hero min-h-screen flex items-center relative overflow-hidden pt-24">
            ...
            <div className="absolute inset-0 overflow-hidden">
              <div className="blob absolute w-96 h-96 rounded-full" 
                   style={{ background: '#FF7A2D', top: '15%', left: '10%', opacity: '0.2' }} />
              <div className="blob absolute w-80 h-80 rounded-full" 
                   style={{ background: 'var(--primary)', bottom: '10%', right: '5%', animationDelay: '2s' }} />
            </div>
                ...
                >
                  <div className="absolute inset-0 p-12 flex flex-col items-center justify-center gap-12">
                    ...
                    <div className="relative w-48 h-48">
                      <div className="absolute top-0 left-12 w-24 h-24 rounded-full opacity-40" 
                           style={{ background: 'radial-gradient(circle, #FF7A2D 0%, transparent 70%)', animation: 'float 4s ease-in-out infinite' }} />
                      <div className="absolute top-12 right-0 w-32 h-32 rounded-full opacity-30" 
                           style={{ background: 'radial-gradient(circle, var(--primary) 0%, transparent 70%)', animation: 'float 4s ease-in-out infinite 1s' }} />
                      <div className="absolute bottom-8 left-0 w-28 h-28 rounded-full opacity-35" 
          ...
          <section className="py-32 relative overflow-hidden" style={{ background: 'var(--bg)' }}>
            ...
            <div className="absolute top-20 left-10 w-96 h-96 rounded-full opacity-5 pointer-events-none" 
                 style={{ background: 'radial-gradient(circle, #FF7A2D 0%, transparent 70%)', animation: 'float 15s ease-in-out infinite' }} />
            <div className="absolute bottom-40 right-20 w-[500px] h-[500px] rounded-full opacity-5 pointer-events-none" 
                 style={{ background: 'radial-gradient(circle, var(--primary) 0%, transparent 70%)', animation: 'float 20s ease-in-out infinite 3s' }} />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full opacity-3 pointer-events-none" 
                 style={{ background: 'radial-gradient(circle, #FF7A2D 0%, transparent 70%)', animation: 'float 25s ease-in-out infinite 6s' }} />
    ...
    function Router() {
      ...
      if (path === '/showcase/bakery') return <BakeryShowcase />;
      if (path === '/showcase/techstore') return <TechStoreShowcase />;
      if (path === '/showcase/restaurant') return <RestaurantShowcase />;
      return <HomePage />;
    }
